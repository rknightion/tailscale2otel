...
#
# The commands here mirror CI exactly (.github/workflows/helm.yml and the
# metricscatalog step in CLAUDE.md) so local output is byte-identical to what CI
# expects. Run it by hand (`scripts/regen-generated.sh`) or let the pre-commit
# hook (.githooks/pre-commit) call it with just the targets your staged changes
# touched.
#
# Usage:
...
main() {
  ...
  local do_docs=0 do_schema=0 do_metrics=0
  for t in "${targets[@]}"; do
    case "$t" in
      all)         do_docs=1; do_schema=1; do_metrics=1 ;;
