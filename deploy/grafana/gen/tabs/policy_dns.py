...
def tab_policy_dns(scope):
    ...
    dns = [
        (panel("MagicDNS", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_magic_dns_ratio"), WIN_SLOW))],
               mappings=BOOL_NEUTRAL, options=stat_opts(),
               novalue="No DNS configuration data. Requires collectors.dns.enabled and a "
                       "credential that can read tailnet DNS settings.",
        ...
        (panel("Nameservers", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_nameservers_count_ratio"), WIN_SLOW))],
        ...
        (panel("Search paths", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_search_paths_count_ratio"), WIN_SLOW))],
               unit="short", options=stat_opts(),
        ...
        (panel("Split-DNS zones", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_split_zones_count_ratio"), WIN_SLOW))],
               unit="short", options=stat_opts(),
        ...
        (panel("Override local DNS", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_override_local_ratio"), WIN_SLOW))],
               mappings=BOOL_NEUTRAL, options=stat_opts(),
               novalue="No DNS configuration data. Requires collectors.dns.enabled and a "
                       "credential that can read tailnet DNS settings.",
        ...
        (panel("Exit-node resolvers", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_dns_resolvers_use_with_exit_node_ratio"), WIN_SLOW))],
               unit="short", options=stat_opts(),
        ...
        (panel("Search domains", "barchart",
               [prom_t("count by (tailscale_dns_search_path_domain) (%s)" % lot(sel("tailscale_dns_search_path_ratio"), WIN_SLOW),
        ...
        (panel("Resolvers", "table",
               [prom_t("%s" % lot(sel("tailscale_dns_resolver_ratio"), WIN_SLOW), instant=True, fmt="table")],
    ...
    settings = [
        ...
        (panel("Tailnet settings", "table",
               [prom_t(lot(sel("tailscale_setting_enabled_ratio"), WIN_SLOW), instant=True, fmt="table")],
        ...
        (panel("Device key duration", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_setting_devices_key_duration_days"), WIN_SLOW))],
        ...
        (panel("Tailnet features", "table",
               [prom_t(lot(sel("tailscale_feature_enabled_ratio"), WIN_SLOW), instant=True, fmt="table")],
               mappings=BOOL_NEUTRAL,
               transformations=[organize(exclude=_INFRA_TBL,
                                         rename={"tailscale_feature": "Feature", "Value": "Enabled"})],
               desc="Per-feature enabled (1) / disabled (0)."), 12, 7),
        # Task 1H.8 — External-tailnets role
        (panel("External-tailnets role", "table",
               [prom_t(lot(sel("tailscale_setting_users_external_tailnets_role_ratio"), WIN_SLOW),
                       instant=True, fmt="table")],
               transformations=[organize(exclude=_INFRA_TBL + ["Value"],
                                         rename={"tailscale_setting_role": "Role"})],
               desc="Role granted to users joining from external tailnets. "
                    "Values: none / member / admin. This is configuration, not a health verdict."), 6, 5),
        ...
        (panel("Webhook endpoints", "stat",
               [prom_t("max(%s)" % lot(sel("tailscale_webhook_endpoints_count_ratio"), WIN_SLOW))],
               unit="short", options=stat_opts(),
