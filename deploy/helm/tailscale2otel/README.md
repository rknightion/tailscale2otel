...
Ready standbys remain available for process telemetry, while every enabled
per-listener Service selects `tailscale2otel.m7kni.io/role: leader` in addition
to the normal chart labels. The elected pod sets that label while it holds the
Lease; its absence means a pod is not the leader. The headless Service, workload
selector, PodMonitor, ServiceMonitor, and NetworkPolicy continue to select every
replica.

This requires an application image that implements the leader label contract:
use the release candidate carrying TSO-0144 or a later image. An older image
with this chart in coordinated mode never labels a pod, so enabled listener
Services have no endpoints.

The chart creates a separate Role and RoleBinding in the release namespace for
the chart ServiceAccount to `get` and `patch` pods. Kubernetes RBAC cannot dynamically
scope `patch` to the calling pod's name, so this grant covers pods throughout that namespace;
the coordinator only changes its identity pod's leader role label. The existing
coordination Role remains scoped to the configured Lease/checkpoint namespace.

The StatefulSet keeps `podManagementPolicy: Parallel` and uses Kubernetes'
default `RollingUpdate` strategy. GitOps image changes and Helm upgrades replace
pods normally, and `helm --wait`/`--atomic` can wait for every Ready replica. On
a leadership hand-off, listener Service endpoints move to the new leader within
the Lease timing. There can be a brief interval with no listener endpoint while
the new leader applies its label.
...
| config.collectors.pam.snapshot_heartbeat | string | `"24h"` | Refresh unchanged snapshots at this cadence. |
...
| config.coordination.renew_deadline | string | `"10s"` | Time allowed to renew before stepping down. Must be greater than 1.2 * retry_period. |
...
| config.webhook.secret | string | `""` | HMAC-SHA256 verification secret. Empty is accepted only on loopback; otherwise the configuration fails startup validation. Set via TS2OTEL_WEBHOOK__SECRET (secret). |
...
| extraContainers | list | `[]` | Extra SIDECAR containers appended to the pod spec as-is, after the exporter. For a companion process that has to share the exporter's network namespace or its volumes — the motivating case is a `tailscale` sidecar giving the pod tailnet membership, which is what config.collectors.node_metrics.discovery needs to reach devices on 100.64.0.0/10 (no Kubernetes egress mechanism expresses "every address in the tailnet", and a per-target ExternalName Service does not scale to a DISCOVERED target set).  Each entry is a standard Kubernetes Container and is rendered VERBATIM — the chart adds no securityContext, resources, probes, volumeMounts or env of its own. A sidecar needing what the exporter drops must say so in its OWN container securityContext, which overrides podSecurityContext; do not weaken podSecurityContext to suit a sidecar, or the exporter silently loses the same hardening.  Names must be unique and must not collide with the exporter's own container ("tailscale2otel"); rendering FAILS rather than producing a Pod the API server rejects.  TWO WAYS TO RUN A TAILSCALE SIDECAR, and the cheap one is usually enough:    * USERSPACE (TS_USERSPACE=true, the image default) needs NO capabilities and NO host     device. It gives no routes, so the exporter reaches the tailnet through the sidecar's     outbound HTTP proxy: set TS_OUTBOUND_HTTP_PROXY_LISTEN and point the exporter's     HTTP_PROXY at it via extraEnv. That covers node-metrics scraping, which is plain HTTP.     Set HTTP_PROXY only — never HTTPS_PROXY — or the exporter's OTLP, Tailscale API,     Pyroscope and MaxMind traffic is also sent at the tailnet, where it has no route.   * KERNEL (TS_USERSPACE=false) gives the pod real 100.64.0.0/10 routes, so nothing needs     proxy configuration, but it needs NET_ADMIN *and* access to /dev/net/tun — an     extraVolumes hostPath plus a matching volumeMount here, or a TUN device plugin.     Tailscale's own Kubernetes examples run this container `privileged: true`.  Example (userspace; pair with extraEnv HTTP_PROXY: http://localhost:1055):   extraContainers:     - name: tailscale       image: ghcr.io/tailscale/tailscale:v1.102.3       env:         - name: TS_AUTHKEY           valueFrom:             secretKeyRef: {name: my-ts-secret, key: TS_AUTHKEY}         - name: TS_OUTBOUND_HTTP_PROXY_LISTEN           value: localhost:1055       securityContext:         capabilities:           drop: ["ALL"]  SHUTDOWN ORDER. Kubernetes sends SIGTERM to all containers in a Pod at the same time; it does not stop the exporter before these sidecars. The exporter has a 40-second staged drain/flush budget (receivers, ingress WAL, OTLP and flow stores), while the chart's default terminationGracePeriodSeconds is 55. A userspace proxy or Tailscale sidecar must therefore tolerate that window; if it must stay available while the exporter drains, give it an appropriate preStop/lifecycle policy and keep the Pod grace period long enough for both containers. Do not assume sidecar declaration order provides shutdown order. |
| extraEnv | list | `[]` |  |
...
| networkPolicy.egress.allowAll | bool | `true` | Allow ALL egress (the default). READ THIS before setting it false. Kubernetes NetworkPolicy has no hostname/DNS-name selector — only podSelector, namespaceSelector and ipBlock — so there is no portable way to express "the Tailscale control-plane API" (Tailscale publishes no stable IP range Tailscale itself commits to) or "an operator-chosen OTLP endpoint" (arbitrary, different per install). CNIs that add hostname-based egress (Cilium's CiliumNetworkPolicy, Calico's GlobalNetworkPolicy) solve this, but are NOT the portable networking.k8s.io/v1 API this chart emits, so this chart does not attempt one and will not invent an IP allowlist that would silently go stale. This policy's real value is its INGRESS half below, which CAN be made portable and correct — egress defaults to allowAll for exactly that reason. Setting this false keeps only egress.dns (if true) plus egress.extra, and WILL BREAK the exporter — it can reach neither the Tailscale API nor your OTLP endpoint — unless egress.extra covers both. If you need a real egress restriction, use your CNI's own policy CRD instead. For example, a CiliumNetworkPolicy can allow MaxMind by hostname (add equivalent rules for the API, OTLP and object-store hosts):    apiVersion: cilium.io/v2   kind: CiliumNetworkPolicy   metadata: {name: tailscale2otel-fqdn-egress}   spec:     endpointSelector: {matchLabels: {app.kubernetes.io/name: tailscale2otel}}     egress:       - toFQDNs: [{matchName: download.maxmind.com}]         toPorts: [{ports: [{port: "443", protocol: TCP}]}] A Calico GlobalNetworkPolicy has the equivalent domain selector:    apiVersion: projectcalico.org/v3   kind: GlobalNetworkPolicy   metadata: {name: tailscale2otel-fqdn-egress}   spec:     selector: "app.kubernetes.io/name == 'tailscale2otel'"     egress:       - action: Allow         destination: {domains: [download.maxmind.com], ports: [443]}         protocol: TCP |
| networkPolicy.egress.dns | bool | `true` | Allow DNS (UDP+TCP port 53) egress. Almost always required: the pod resolves the Tailscale API host, the OTLP endpoint host, and (if configured) an S3-compatible object-store endpoint by name. If enrichment.geoip.download.enabled is true, it also needs HTTPS egress to download.maxmind.com (or the hostname in enrichment.geoip.download.endpoint). Only consulted while allowAll is false. |
...
| secret.TS2OTEL_PAM__TOKEN | string | `""` | Static read-only Border0 service-account bearer token for the optional PAM collector. |
...
| secret.TS2OTEL_WEBHOOK__SECRET | string | `""` | Webhook HMAC-SHA256 secret. Set ONLY when you enable config.webhook. Empty is accepted only on a loopback config.webhook.listen; an enabled network-reachable listener without a route secret fails startup validation. |
...
| terminationGracePeriodSeconds | int | `55` | Seconds the kubelet allows for a graceful stop before SIGKILL. Shutdown is STAGED and each stage is separately bounded in the binary: the receivers drain already-ACKed requests (10s), the ingress WAL performs one final drain (10s), then the OTLP pipeline flushes and shuts down (10s), and the flow stores close concurrently under one shared wait budget (10s) — 40s worst case, plus headroom. Kubernetes' own default is 30s, which is below the drain, so a rollout could SIGKILL the pod mid-flush and lose the final flow rollup, the WAL backlog and the last export. Values below the enforced minimum make `helm template` fail rather than silently truncating a drain. Raise it if you raise a stage timeout; `internal/app` fails the build if the two stop agreeing (#332). |
