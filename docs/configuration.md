...
suggests it. Keys under a dynamic map (`otlp.headers.*`, a node-metrics target's
...
- [High availability](high-availability.md) and [`coordination`](#coordination-kubernetes-active-passive-operation)
- [`pam`: PAM connection](#pam-tailscale-pam-border0-api-connection)
- [`scheduler`: initial tick spread](#scheduler-initial-tick-spread)
...
`source: poll` or `both`) has read, so a restart resumes without gaps or large overlaps. Object-store identities and bounded dedup state can also use checkpoints; streaming does not
make every checkpoint namespace unused. The same atomic file also carries
...
- **`kubernetes`**: cursor and dedup state is compressed into per-collector ConfigMap shards,
  named from the Lease and a shard hash. Writes are coalesced and flushed on orderly shutdown.
  Updates use resource-version conflict checks; each compressed shard must fit the ConfigMap
  limit. Startup rejects configured capacities that cannot fit. This mode requires
  `coordination.mode: kubernetes` and uses `coordination.namespace`. The legacy
  `<lease>-checkpoints` object is retained for migration and rollback reconciliation. See
  [High availability](high-availability.md) for local-state limits and rollout requirements.
...
> response deadline as defence in depth. The record and depth caps are the required controls: the
