...
| `tailscale2otel.dedup.youngest_eviction_age` | `s` | gauge | `tailscale2otel_dedup_youngest_eviction_age_seconds` | `dedup_set` | Smallest residency age in seconds observed for a key evicted from a de-duplication set because it reached capacity, by set. Absent until the first eviction; values below the poll overlap horizon indicate the set is undersized. |
