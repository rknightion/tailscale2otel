...
| `tailscale.pam.session` | INFO | `tailscale_pam_service_name`, `tailscale_pam_session_type`, `tailscale_pam_session_authorization_result`, `tailscale_pam_session_killed`, `tailscale_pam_session_recording_type`, `tailscale_pam_session_duration_seconds`, `user_name`, `user_full_name`, `user_id`, `source_address`, `source_port`, `host_name`, `tailscale_pam_session_command` | One record for each newly accepted PAM session when `collectors.pam.session_log_enabled` is set. `tailscale.pam.session.authorization_result` is the authorization outcome, not connection health: a session can be authorized even if its upstream connection later fails. A tailnet grant-layer denial produces no session row and therefore no record. Identity, address, device-name, and command attributes follow the configured `pii_filter` categories; `auth_info` and event metadata are never emitted. |
