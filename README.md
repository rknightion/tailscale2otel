...
tailscale2otel exports Tailscale fleet state, network flows and configuration changes as
OpenTelemetry metrics and logs over OTLP, through a Prometheus `/metrics` endpoint, or both.
It runs as one static Go binary and supports a reduced collector set for
[Headscale](https://headscale.net/). Optional traces describe the exporter’s own work.
...
Tailscale exposes device inventory, flow and audit logs, users, keys, DNS, ACL policy and device
posture through its APIs. tailscale2otel combines those sources in your existing telemetry backend
and derives expiry, traffic, configuration and collector-health signals. Cardinality controls bound
the number of flow metric series.
...
- **Four ingestion paths into one pipeline** - poll the API, receive Tailscale's log stream on a
  ...
  S3-compatible bucket, or take real-time HMAC-verified webhooks. Flow and audit sources share their respective processors; webhooks have their own event processor.
- **Offline GeoIP and ASN enrichment of external peers.** Optional, from MaxMind `.mmdb` files on
  ...
  locality and coordinates) ride the flow logs, without adding metric labels. Databases hot-swap on
...
- **PII controls** cover 13 categories, including emails, user IDs, hostnames, IPs, node IDs and
  free-text detail. Categories retain their values by default; set a category to `false` to redact
  it from exported metric attributes, log records and spans. The [security
  guide](https://m7kni.io/tailscale2otel/security/) explains local storage and raw-policy exceptions.
...
- **OTLP push** (`otlp.protocol: grpc|http`) with Grafana Cloud authentication: set
...
- **Local explorers** provide a [flow view](https://m7kni.io/tailscale2otel/flow-view/),
  an [event explorer](https://m7kni.io/tailscale2otel/events/), JSON/CSV flow exports, and a
  [support bundle](https://m7kni.io/tailscale2otel/troubleshooting/#generating-a-support-bundle).
  They use the admin authentication boundary.
- **Continuous profiling** is opt-in. pprof requires the admin server and an admin token;
  Pyroscope can push profiles to a separate backend.
- **Kubernetes high availability** uses a Lease to elect one active replica from two or three
  pods. See [High availability](https://m7kni.io/tailscale2otel/high-availability/) for shared
  checkpoints, local state and rollout requirements.
...
token exchange, e.g. a Kubernetes projected service-account token) is also supported, and supported
file-backed credentials have a `*_file` variant for Docker/Kubernetes secrets. PAM uses
`TS2OTEL_PAM__TOKEN` and has no `token_file` setting.

OAuth access tokens refresh automatically. Selected TLS and credential files can rotate in place;
other configuration changes require a restart. See the [reload
reference](https://m7kni.io/tailscale2otel/configuration/#reload-classifications).
...
`enrich` (device cache), `rdns`, `config`, and the `stream`/`webhook` receivers. Generated artifacts have family-specific
`gen-*` recipes. Run `just gen` or the affected family when changing their inputs. `just --list`
shows the supported tasks; `just check` covers the root and four tool modules.

## API drift CI

Tailscale's API and OpenAPI spec evolve continuously ("may change or break without notice"), which
has broken decoders here before. The repository runs these checks:
...
| **Exploratory fuzzing** | pushes to main (advisory) | `go test -fuzz` over the HEC envelope, HEC timestamps and the flow/audit decoders. Deliberately **not** required: finding a NEW crasher is nondeterministic, so gating it would let an unrelated PR randomly block merges. Each target's seed corpus runs in the gated leg above, so a KNOWN crasher still blocks |
